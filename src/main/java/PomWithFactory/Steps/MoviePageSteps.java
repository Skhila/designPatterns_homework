...
import PomWithFactory.Pages.MoviePage;
...
public class MoviePageSteps {
    ...
    public MoviePageSteps clickOnLastDate(){
        performScrollAndClick(moviePage.eastPointLastDate, js);
        return this;
    }
    ...
    public MoviePageSteps validateSeances(){
        ...
        for (WebElement seance : moviePage.caveaSeances) {
    ...
    public String getMovieNameBefore(){
        return moviePage.movieNameElement.getText();
    }

    @Step("Get seance date and time before selecting seance")
   public String getSeanceDateTimeBefore(){
        WebElement caveaLastSeance = moviePage.caveaSeances.get(moviePage.caveaSeances.size()-1);
        String seanceDateBefore = moviePage.eastPointLastDate.getText().split(" ")[0];
